...
const loadPortfolioSnapshot = async (): Promise<PortfolioSnapshot | null> => {
  try {
    if (isExtension()) {
      ...
      const snapshot = result[PORTFOLIO_STORAGE_KEY] as PortfolioSnapshot | undefined;
...
export const calculatePortfolio24hChange = async (currentValueUsd: number): Promise<number> => {
  ...
  if (snapshot && now - snapshot.timestamp >= oneDayMs - (60 * 60 * 1000)) {
    ...
    if (snapshot.valueUsd > 0) {
      const change = ((currentValueUsd - snapshot.valueUsd) / snapshot.valueUsd) * 100;
      return change;
    }
  }
