...
export const saveWallet = async (data: EncryptedData, token?: string): Promise<void> => {
  if (isExtension()) {
...
export const loadWallet = async (): Promise<EncryptedData | undefined> => {
  if (isExtension()) {
    const result = await chrome.storage.local.get([WALLET_STORAGE_KEY]);
    return result[WALLET_STORAGE_KEY] as EncryptedData | undefined;
  } else {
    // Fallback
    const stored = localStorage.getItem(WALLET_STORAGE_KEY);
    return stored ? JSON.parse(stored) as EncryptedData : undefined;
  }
};
